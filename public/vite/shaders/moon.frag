...
uniform sampler2D logo; // data/logo.jpg
uniform vec2 logoResolution;
...
vec4 sphereTexture(in sampler2D _tex, in vec2 _uv, float _time) {
    ...
    float aspect = tex0Resolution.y/tex0Resolution.x;
...
void main(){
    ...
    vec3 color = vec3(0.0);
    color = sphereTexture(tex0, st, time * 0.01).rgb;

    // Calculate sun direction
    float speedSun = 0.25;
    vec3 sunPos = normalize(vec3(cos(time * speedSun - HALF_PI), 0.0, sin(speedSun * time - HALF_PI)));
    ...
    st = scale(st, 2.);
    color += texture2D(logo, st).rgb * step(.0001,st.y) * step(st.y,.999);
