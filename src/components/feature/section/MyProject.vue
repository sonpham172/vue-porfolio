      ...
      <swiper-slide class="slide skill-item" v-for="item in PROJECTS">
        ...
        <div class="flex flex-col gap-4 md:gap-8">
          ...
          <div class="flex flex-col gap-2">
            ...
            <a v-if="item.link !== ''" :href="item.link" target="_blank" class="text-[burlywood] underline">{{item.link}}</a>
            <span class="text-white" v-else>Update later ...</span>
          </div>

          <BasicButton value="See detail" :isLink="true" :path="`/project/${item.id}`" />
        </div>
      </swiper-slide>
    </swiper>
